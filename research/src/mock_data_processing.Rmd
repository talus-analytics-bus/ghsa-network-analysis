...
fc_map <- read.table("../data/aggregate/recipient_country_mapping.txt", header = TRUE, sep = "\t")
...
The sampled dataset used as part of the demo dataset will include data from the following sources:  
  * GAVI (random sample of 100 recipient-application pairs form years 2012-2015)
...
gavi_sample_indices <- sample(which(core_data$year %in% c(2012:2015) &
                                    core_data$data_source == "GAVI"), size = 100)
...
## code any remaining NAs as "unspecified"
...
## sample from 25th-60th quantile
...
## sample from 20th-40th quantile
...
## sample from 30th-70th quantile
...
```{r}
## merge actual and notional data
...
Ensure that simulated notional data isn't too far out of line.

```{r}
## still needs legend
ggplot(demo_data, aes(log(amount_dispersed), fill = data_source)) +
    geom_histogram(bins = 25) +
  ylab("Number of Dispers") +
...
ggplot(demo_data, aes(log(amount_dispersed))) +
    geom_histogram(data = subset(demo_data, core_element == 'Prevent'), fill = "red", alpha = 0.2) +
    geom_histogram(data = subset(demo_data, core_element == 'Detect'), fill = "blue", alpha = 0.2) +
    geom_histogram(data = subset(demo_data, core_element == 'Respond'), fill = "green", alpha = 0.2) +
    geom_histogram(data = subset(demo_data, core_element == 'Other'), fill = "orange", alpha = 0.2) +
